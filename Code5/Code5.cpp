...
//着色函数
Vector3 Color(const Vector3 &Pixel, Triangle &triangle)
{
	Vector3 Color(0, 0, 0);
	...
	if (weight1 >= 0 && weight2 >= 0 && weight3 >= 0)
	{
		...
		z = 1.0 / z;
		//对顶点属性进行插值
		Color = (triangle.v1.Color*weight1*z1Inverse 
		+ triangle.v2.Color*weight2*z2Inverse 
		+ triangle.v3.Color*weight3*z3Inverse)*z;
		return Color;
	}
	return Color;
...
int main()
{
	...
	Vertex vertA(vertPosA, Vector3(250, 0, 0));
	Vertex vertB(vertPosB, Vector3(0, 250, 0));
	Vertex vertC(vertPosC, Vector3(0, 0, 250));

	Triangle triangle(vertA, vertB, vertC);
	ProjectTriangleTo2D(triangle);
	ofstream outfile("Code5.ppm");
	...
	for(int i=0;i<imageWidth;i++)
	{
		for(int j=0;j<imageHeight;j++)
		{
			...
			image[i][j] = Color(Pixel, triangle);
		}
	}
