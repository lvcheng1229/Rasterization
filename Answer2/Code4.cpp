...
int main()
{
	...
	Vector3 vertPosB(0, imageHeight, 0);
	Vector3 vertPosC(imageWidth, 0, 0);

	
	Vector3 vertPosD(0, imageHeight, 0);
	Vector3 vertPosE(imageWidth, imageHeight, 0);
	Vector3 vertPosF(imageWidth, 0, 0);
	...
	for(int i=0;i<imageWidth;i++)
	{
		...
		{			
			...
			float weight6 = EdgeFunction(vertPosD, vertPosE, Pixel)/2.0;
			float weight4 = EdgeFunction(vertPosE, vertPosF, Pixel)/2.0;
			float weight5 = EdgeFunction(vertPosF, vertPosD, Pixel)/2.0;
			...
			{
				image[i][j] = Vector3(0, 0, 0);
			}

			if (weight4 >= 0 && weight5 >= 0 && weight5 >= 0)
			{
				float triangleArea = EdgeFunction(vertPosD, vertPosE,vertPosF)/2.0;

				//计算重心坐标
				weight4 /= triangleArea;
				weight5 /= triangleArea;
				weight6 /= triangleArea;

				image[i][j] = ColorA * weight4 + ColorB * weight5 + ColorC * weight6;
			}
