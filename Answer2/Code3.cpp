...
int main()
{
	...
	Vector3 vertPosA(70, 0, 0);
	Vector3 vertPosB(50, 50, 0);
	Vector3 vertPosC(100, 100, 0);
	Vector3 vertPosD(150, 50, 0);
	Vector3 vertPosE(130, 0, 0);

	ofstream outfile("Code3.ppm");
	...
	for(int i=0;i<imageWidth;i++)
	{
		...
		{			
			...
			{
				image[i][j] = Vector3(255, 0, 0);
			}
			else
			{
				image[i][j] = Vector3(50, 50, 50);
			}*/
			float weight1 = EdgeFunction(vertPosA, vertPosB, Pixel)/2.0;
			float weight2 = EdgeFunction(vertPosB, vertPosC, Pixel)/2.0;
			float weight3 = EdgeFunction(vertPosC, vertPosD, Pixel)/2.0;
			float weight4 = EdgeFunction(vertPosD, vertPosE, Pixel)/2.0;
			float weight5 = EdgeFunction(vertPosE, vertPosA, Pixel)/2.0;
			//如果五个重心坐标都大于0则该像素点在三角形内
